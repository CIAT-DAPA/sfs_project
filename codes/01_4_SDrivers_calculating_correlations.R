...
#                    legend.title = element_blank(),
#                    legend.text  = element_text(size = 15))
#   outfile <- paste0(data_path,"/drivers_graphs/correlations/v1/SFS_index_vs_",driver,"_removing_outliers.png")
#   if(!file.exists(outfile)){
...
# Version 2.0: GAM curves (removing minimum value)
calc_best_curve <- function(df = sfs_df, driver = drivers_list[1], label = drivers_lbls$label[1], remove_min = T){
  ...
  gp2 <- jcknf_smpl %>%
    ...
    ggplot2::theme(axis.title   = element_text(size = 15),
                   ...
                   legend.title = element_blank(),
                   legend.text  = element_text(size = 13))
  outfile <- paste0(data_path,"/drivers_graphs/correlations/v2/SFS_index_vs_",driver,"_removing_outliers.png")
  if(!file.exists(outfile)){
    ggplot2::ggsave(filename = outfile, plot = gp2, device = "png", units = "in", width = 8, height = 8)
  }
...
# Version 2.1: Polynomial 2 curves (removing minimum value)
calc_best_curve_poly <- function(df = sfs_df, driver = drivers_list[1], label = drivers_lbls$label[1], remove_min = T){
  ...
  gp2 <- jcknf_smpl %>%
    ggplot2::ggplot(aes(x, y, group = id)) +
    ggplot2::geom_line(alpha = 0.5, colour = "dodgerblue2") + #I(1/sqrt(nrow(df_fltr_updt)))
    ggplot2::geom_point(data = df_fltr_updt %>% mutate(id = 0), aes(x, y)) +
    ggplot2::xlab(label) +
    ggplot2::ylab("Country food system sustainability scores") +
    ggplot2::ylim(.23, .75) +
    ...
    ggplot2::theme(axis.title   = element_text(size = 15),
                   ...
                   legend.title = element_blank(),
                   legend.text  = element_text(size = 13))
  outfile <- paste0(data_path,"/drivers_graphs/correlations/v3/SFS_index_vs_",driver,"_removing_outliers_poly.png")
  if(!file.exists(outfile)){
    ggplot2::ggsave(filename = outfile, plot = gp2, device = "png", units = "in", width = 8, height = 8)
  }
...
drivers_best_curve <- 1:length(drivers_list) %>% purrr::map(function(i) calc_best_curve_poly(df     = sfs_df,
                                                                                             driver = drivers_list[i],
                                                                                             label  = drivers_lbls$label[i],
                                                                                             remove_min = T))
...
# Version 2.2: Polynomial curves (removing different number of observations)
calc_best_curve_poly <- function(df = sfs_df, driver = drivers_list[1], label = drivers_lbls$label[1], remove_min = T){
  ...
  # jcknf_smpl <- lapply(1:(nrow(df_fltr_updt)-2), function(i){
  #   df_flt    <- df_fltr_updt[-c(i,nrow(df_fltr_updt)-1,nrow(df_fltr_updt)),] # df_fltr_updt[-c(1,i),]
  #   fit       <- lm(y ~ poly(x, 3), data = df_flt)
  #   df_flt$y  <- fit$fitted.values
  #   df_prd    <- df_fltr_updt[c(nrow(df_fltr_updt)-1,nrow(df_fltr_updt)),]
  #   df_prd$y  <- predict(fit, df_fltr_updt[c(nrow(df_fltr_updt)-1,nrow(df_fltr_updt)),])
  #   df_flt    <- rbind(df_flt, df_prd)
  #   df_flt$id <- i
  #   return(df_flt)
  # })
  
  # jcknf_smpl <- lapply(2:nrow(df_fltr_updt), function(i){
  #   df_flt    <- df_fltr_updt[-c(1, i),] # df_fltr_updt[-c(1,i),]
  #   fit       <- lm(y ~ poly(x, 2), data = df_flt)
  #   df_flt$y  <- fit$fitted.values
  #   df_prd    <- df_fltr_updt[1,]
  #   df_prd$y  <- predict(fit, df_fltr_updt[1,])
  #   df_flt    <- rbind(df_flt, df_prd)
  #   df_flt$id <- i
  #   return(df_flt)
  # })
  ...
  ## Services and trade
  jcknf_smpl <- lapply(1:(nrow(df_fltr_updt)-3), function(i){
    df_flt    <- df_fltr_updt[-c(i,nrow(df_fltr_updt)-2,nrow(df_fltr_updt)-1,nrow(df_fltr_updt)),] # df_fltr_updt[-c(1,i),]
    fit       <- lm(y ~ poly(x, 2), data = df_flt) # 3 originally
    df_flt$y  <- fit$fitted.values
    df_prd    <- df_fltr_updt[c(nrow(df_fltr_updt)-2,nrow(df_fltr_updt)-1,nrow(df_fltr_updt)),]
    df_prd$y  <- predict(fit, df_fltr_updt[c(nrow(df_fltr_updt)-2,nrow(df_fltr_updt)-1,nrow(df_fltr_updt)),])
    df_flt    <- rbind(df_flt, df_prd)
    df_flt$id <- i
    return(df_flt)
  })
  
  # jcknf_smpl <- lapply(8:nrow(df_fltr_updt), function(i){
  #   df_flt    <- df_fltr_updt[-unique(c(1:7,i)),] # df_fltr_updt[-c(1,i),]
  #   fit       <- lm(y ~ poly(x, 2), data = df_flt)
  #   df_flt$y  <- fit$fitted.values
  #   df_prd    <- df_fltr_updt[1:7,]
  #   df_prd$y  <- predict(fit, df_fltr_updt[1:7,])
  #   df_flt    <- rbind(df_flt, df_prd)
  #   df_flt$id <- i
  #   return(df_flt)
  # })
  ...
  gp2 <- jcknf_smpl %>%
    ...
    ggplot2::theme(axis.title   = element_text(size = 15),
                   ...
                   legend.title = element_blank(),
                   legend.text  = element_text(size = 13))
  outfile <- paste0(data_path,"/drivers_graphs/correlations/v3/SFS_index_vs_",driver,"_removing_outliers_poly2.png")
  if(!file.exists(outfile)){
    ggplot2::ggsave(filename = outfile, plot = gp2, device = "png", units = "in", width = 8, height = 8)
  }
...
calc_best_curve_poly(df = sfs_df, driver = drivers_list[4], label = drivers_lbls$label[4], remove_min = T)
c(7,22)
